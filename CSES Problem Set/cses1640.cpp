...
*/
    ...
    */
    ...
    vector <pair <ll , ll>> marr;
    for (ll i = 1; i <= n; i++){
        ll j; cin >> j;
        marr.push_back(pair <ll , ll>(j , i));
    }

    ll flag = 0;
    sort(marr.begin() , marr.end());
    for (ll i = 0; i < n; i++){
        //binary search x - marr[i] in marr
        ll val = x - marr[i].first , pos = -1;
        ll lo = 0 , hi = n - 1;
        while(lo <= hi){
            ll mid  = lo + (hi - lo) / 2;

            if (marr[mid].first == val)
                pos = mid;
            if (marr[mid].first < val)
                lo = mid + 1;
            else
        ...
        if (pos != -1 && pos != i){
            flag = 1;
            cout << marr[i].second << " " << marr[pos].second;
            break;
        }
    }
    if (flag == 0)
