...
/*
	 ...
	 date created: 2021-08-23 22:55:03
	 problem link: 
*/
#include<iostream>
#include<vector>
...
ldb expectedValue(ll c, ll m, ll p){
    ...
    //simulate picking of m
    if (m > 0){
        if (m <= volatility){
            if (c == 0)
                Ex = Ex + ((ldb)m/scale)*(1 + expectedValue(0, 0, p + m));
            else
                Ex = Ex + ((ldb)m/scale)*(1 + expectedValue(c + m/2, 0, p + m/2));
        }
        else{
            if (c == 0){
                Ex = Ex + ((ldb)m/scale)*(1 + expectedValue(0, m - volatility, p + volatility));
            }
            else{
                Ex = Ex + ((ldb)m/scale)*(1 + expectedValue(c + volatility/2, m - volatility, p + volatility/2));
            }
        }
    }
    ...
    if (c > 0){
        if (c <= volatility){
            if (m == 0)
                Ex = Ex + ((ldb)c/scale)*(1 + expectedValue(0, 0, p + c));
            else
                Ex = Ex + ((ldb)c/scale)*(1 + expectedValue(0, m + c/2, p + c/2));
        }
        else{
            if (m == 0){
                Ex = Ex + ((ldb)c/scale)*(1 + expectedValue(c - volatility, 0, p + volatility));
            }
            else{
                Ex = Ex + ((ldb)c/scale)*(1 + expectedValue(c - volatility, m + volatility/2, p + volatility/2));
            }
        }
    }
    return Ex;
...
void solve(ll mcase){
    ...
    ll c1 = round(scale*c), m1 = round(scale*m), p1 = round(scale*p); 
    volatility = round(scale*v);
