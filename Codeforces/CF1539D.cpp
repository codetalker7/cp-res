...
/*
	 ...
	 file name: CF1539D
	 date created: 2021-06-22 01:03:26
	 problem link: https://codeforces.com/contest/1539/problem/D
*/
#include<iostream>
#include<vector>
#include<string>
...
void solve(ll mcase){
    ...
    ll bought = 0, front_index = 0, back_index = n - 1;
    ll ans = 0;
    while (front_index <= back_index){
        if (bought >= pairs[back_index].b){
            ans += pairs[back_index].a;
            bought += pairs[back_index].a;
            back_index--;
        }
        else{
            if (pairs[front_index].a <= pairs[back_index].b - bought){
                ans += 2*pairs[front_index].a;
                bought += pairs[front_index].a;
                front_index++;
            }
            else{
                ans += 2*(pairs[back_index].b - bought);
                pairs[front_index].a -= pairs[back_index].b - bought;
                bought += pairs[back_index].b - bought;
            }
        }
    }
    cout << ans << "\n";
