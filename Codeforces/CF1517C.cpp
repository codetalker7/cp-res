...
/*
	 ...
	 file name: CF1517C
	 date created: 2021-06-20 02:00:57
	 problem link: https://codeforces.com/contest/1517/problem/C
*/
#include<iostream>
#include<vector>
#include<string>
...
bool bfs(ll n, ll i){
    //starting node is (i, i)
    //order should be left, then down
    ll total = 1;
    queue<pair<ll , ll> > q;
    q.push({i, i});

    while(!q.empty() && total < color[i][i]){
        pair <ll, ll> v = q.front();
        q.pop();

        ll flag = 0;
        ll x = v.first, y = v.second;
        if (y - 1 >= 1 && flag == 0 && y - 1 < x){
            if (color[x][y - 1] == 0){
                color[x][y - 1] = color[i][i];
                total++;
                q.push({x, y - 1});
                flag = 1;
            }
        ...
        if (x + 1 <= n && flag == 0 && y < x + 1){
            if (color[x + 1][y] == 0){
                color[x + 1][y] = color[i][i];
                total++;
                q.push({x + 1, y});
                flag = 1;
            }
...
void solve(ll mcase){
    ...
    //take the diagonal colors
    for (ll i = 1; i <= n; i++){
        cin >> color[i][i];
    }    


    //do the bfs on each diagonal cell
    for (ll i = 1; i <= n; i++){
        if (bfs(n, i) == 0){
            cout << -1 << "\n";
            return;
