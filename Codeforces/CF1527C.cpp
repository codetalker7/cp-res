...
/*
	 ...
	 file name: CF1527C
	 date created: 2021-05-20 21:30:46
...
void solve(ll mcase){
    ...
    ll ans = 0;
    map <ll , ll> seen_thus_far;

    map <ll , ll>::iterator it;

    for (ll i = n; i >= 1; i--){
        if (i == n){
            seen_thus_far[arr[n]] = n;
            continue;
        }
        //otherwise in the set, find the smallest index 
        //greater than i where arr[i] is present
        it = seen_thus_far.find(arr[i]);
        if (it == seen_thus_far.end()){
            seen_thus_far[arr[i]] = i;
            continue;
        }
        else{
            sum_i[i] = n - (*it).second + 1 + sum_i[(*it).second];
            ans += i*sum_i[i];
            //update map as well
            seen_thus_far[arr[i]] = i;
        }
