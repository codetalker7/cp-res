...
/*
	 ...
	 file name: CF1523A
	 date created: 2021-05-30 20:06:50
...
void solve(ll mcase){
    ...
    for (ll i = 0; i < n; i++){
        if (first_index == -1 && state[i] == '1')
            first_index = i;
    }

    if (first_index == -1){
        //simply print the original state
        cout << state << "\n";
        return;
    }
    ...
    if (first_index > 0){
        if (m >= first_index){
            for (ll j = 0; j < first_index; j++)
                state[j] = '1';
        }
        else{
            for (ll j = first_index - m; j < first_index; j++)
                state[j] = '1';
        }
    }
    ...
    while (curr < n){
        ...
        if (has_next){
            if (j - curr - 1 > 1){
                ...
                if (len % 2 == 0){
                    if (m >= len/2){
                        for (ll k = curr + 1; k < j; k++)
                            state[k] = '1'; 
                    }
                    else {
                        for (ll k = curr + 1; k <= curr + m; k++)
                            state[k] = '1';
                        for (ll k = j - 1; k >= j - m; k--)
                            state[k] = '1';
                    }
                }
                else{
