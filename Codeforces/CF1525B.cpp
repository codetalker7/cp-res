...
/*
	 ...
	 file name: CF1525B
	 date created: 2021-05-16 13:48:23
...
void solve(ll mcase){
    ...
    //taking the input
    //flag = 1 means the array is sorted
    ll flag = 1; 
    //mmax and mmin are the maximum and minimum
    ...
    for (ll i = 1; i <= n; i++){
        ...
        if (arr[i] < mmin)
            mmin = arr[i];
        //if the array is not sorted, unset the flag
    ...
    //else if arr[1] == mmin or arr[n] == mmax
    if (arr[1] == mmin || arr[n] == mmax){
        cout << 1 << "\n";
        return;
    }
    if (arr[1] == mmax && arr[n] == mmin){
        cout << 3 << "\n";
        return;
    }
    cout << 2 << "\n";
