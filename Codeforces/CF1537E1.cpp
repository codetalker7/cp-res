...
/*
	 ...
	 file name: CF1537E1
	 date created: 2021-06-18 21:48:40
	 problem link: https://codeforces.com/contest/1537/problem/E1
*/
#include<iostream>
#include<vector>
#include<string>
...
string get_string(string s, ll i, ll k){
    string temp = "";
    for (ll x = 0; x <= i; x++)
        temp.push_back(s[x]);
...
void solve(ll mcase){
    ...
    for (ll i = 0; i < n; i++){
        if (i + 1 > k)
            break;
        string cand = get_string(s, i, k);
        if (ans == "")
            ans = cand;
        else{
            if (cand.compare(ans) < 0)
                ans = cand;
        }
    }   
    cout << ans << "\n";
