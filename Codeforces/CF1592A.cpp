...
/*
	 ...
	 file name: 1.cpp
	 date created: 2021-10-03 20:05:57
	 problem link: https://codeforces.com/contest/1592/problem/0
*/
#include<iostream>
#include<vector>
#include<string>
...
void solve(ll mcase){
    ...
    //else consider a[n] + a[n - 1]
    else if (a[n] + a[n - 1] >= h){
        printf("2\n");
        return;
    }

    ll times = h / (a[n] + a[n - 1]);
    if (h == times*(a[n] + a[n - 1])){
        printf("%lld\n", 2*times);
        return;
    }
    //h is strictly greater
    if (h <= times*(a[n] + a[n - 1]) + a[n]){
        printf("%lld\n", 2*times + 1);
        return;
    }
    else{
