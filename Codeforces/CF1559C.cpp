...
/*
	 ...
	 file name: 3.cpp
	 date created: 2021-09-01 22:52:15
	 problem link: 
*/
#include<iostream>
#include<vector>
...
void solve(ll mcase){
    ...
    for (ll i = 1; i <= n; i++){
        ...
        if (a == 1){
            min_join = min(min_join, i);
        }
    }

    //the path is 1 -> 2 -> min_join - 1 -> n + 1 -> min_join -> ... -> n
    if (min_join == INF){
        //print the linear path
        for (ll i = 1; i <= n + 1; i++)
            printf("%lld ", i);
        printf("\n");
        return;
        ...
        for (ll i = 1; i <= min_join - 1; i++)
            printf("%lld ", i);
        printf("%lld ", n + 1);
        for (ll i = min_join; i <= n; i++)
            printf("%lld ", i);
        printf("\n");
        return;
    }
