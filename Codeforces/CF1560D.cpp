...
/*
	 ...
	 file name: d.cpp
	 date created: 2021-08-18 20:50:51
	 problem link: 
*/
#include<iostream>
#include<vector>
...
void process(){
    ...
    while(2*curr <= INF){
        ...
        total++;
        powers[total] = curr;
        pow_strings[total] = to_string(curr);
    }  
}
...
void solve(ll mcase){
    ...
    ll ans = INF;
    for (ll i = 1; i <= 60; i++){  
        ll curr_n = 0, curr_i = -1;
        while(curr_n < ssz(str_n) AND curr_i + 1 < ssz(pow_strings[i])){
            if (str_n[curr_n] == pow_strings[i][curr_i + 1]){
                curr_n++;
                curr_i++;
            }
            else
        ...
        ans = min(ssz(str_n) - curr_i - 1 + ssz(pow_strings[i]) - curr_i - 1, ans);
