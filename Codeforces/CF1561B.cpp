...
/*
	 ...
	 file name: CF1561B.cpp
	 date created: 2021-08-25 18:46:08
	 problem link: 
*/
#include<iostream>
#include<vector>
...
ll maxString[200000 + 1];
ll minString[200000 + 1];

void solve(ll mcase){
    ll a, b;
    scanf("%lld %lld", &a, &b);

    //final vector to store the possible values
    vll k(a + b + 1, 0); 
    ll total = 0;
    ...
    //counting the maxCollisions and minCollisions
    ll maxCollisions, minCollisions = abs(As - a);

    //counting maxCollisions
    if (b >= As){
        maxCollisions = As + a;
    } 
    else{
    ...
    for (ll i = minCollisions; i <= maxCollisions; i += 2){
        if (k[i] == 0){
            k[i] = 1;
            total++;
        }
