...
/*
	 ...
	 file name: 6.cpp
	 date created: 2021-10-13 21:20:18
...
void solve(ll mcase){
    ...
    //set non_leaf and is_leaf
    queue <pair<ll, ll>> leaves;
    forll(i, 1, n, 1){
        deg[i] = ssz(adj[i]);
        if (ssz(adj[i]) == 1){
            leaves.push({i, 1});
    ...
    ll ans = 0;
        ...
        while(!leaves.empty() AND ans < n){
            ll u = leaves.front().first, r = leaves.front().second;
