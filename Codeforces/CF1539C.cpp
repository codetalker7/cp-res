...
/*
	 ...
	 file name: CF1539C
	 date created: 2021-06-22 00:32:04
	 problem link: 
*/
#include<iostream>
#include<vector>
...
ll add_x(ll x, ll a, ll b){
    //assume that a < b and b - a > x
    return mceil(b - a, x) - 1;
}
...
void solve(ll mcase){
    ...
    while(curr_index <= n){
        ...
        ll i = curr_index;
        while(i + 1 <= n && a[i + 1] - a[i] <= x){
            i++;
        }

        //pushing i onto index
        index.push_back(i);
        curr_index = i + 1;
    }
    ...
    vll to_add;
    for (ll i = 0; i < ssz(index) - 1; i++){
        to_add.push_back(add_x(x, a[index[i]], a[index[i] + 1]));
    }

    sort(to_add.begin(), to_add.end());

    for (ll i = 0; i < ssz(to_add); i++){
        if (ans == 1){
            cout << 1 << "\n";
            return;
        }
