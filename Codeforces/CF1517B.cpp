...
/*
	 ...
	 file name: CF1517B
	 date created: 2021-06-20 01:30:40
...
void solve(ll mcase){
    ...
    for (ll i = 0; i <= n; i++)
        blank[i] = -1;
    vector <vll> assignments(m + 1, blank);

    //assigning the first m elements
    for (ll i = 0; i < m; i++){
        ll u = nums[i].i, v = nums[i].j;
        assignments[i + 1][u] = v;
    }
    ...
    //setting the rest of the assignments
    ll curr = 0;
    for (ll j = 1; j <= n; j++){
        for (ll i = 1; i <= m; i++){
            if (assignments[i][j] == -1){
                assignments[i][j] = rest[curr].second;
                curr++;
            }
        }
    }
