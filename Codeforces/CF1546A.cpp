...
/*
	 ...
	 file name: CF1546A.cpp
	 date created: 2021-08-15 13:20:57
	 problem link: 
*/
#include<iostream>
#include<vector>
...
void solve(ll mcase){
    ...
    printf("%lld\n", m);
    ll l = 0, h = 0;
    while(l < ssz(lower) AND h < ssz(higher)){
        while(a[lower[l]] < b[lower[l]] AND a[higher[h]] > b[higher[h]]){
            a[lower[l]]++;
            a[higher[h]]--;
            printf("%lld %lld\n", higher[h], lower[l]);
        }
        if (a[lower[l]] == b[lower[l]])
            l++;
        else
            h++;
    }
