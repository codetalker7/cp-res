...
/*
	 ...
	 file name: CF1511C
	 date created: 2021-07-01 22:41:43
...
void solve(ll mcase){
    ...
    for (ll i = 1; i <= n; i++){
        ...
        first_pos[ai] = min(first_pos[ai], i);
    }

    //handling the queries
    for (ll i = 1; i <= q; i++){
        //t is the query color
        ll t;
        cin >> t;

        cout << first_pos[t] << " ";

        //incrementing first_pos of colors above first_pos[i] by 1
        for (ll j = 1; j <= 50; j++){
            if (first_pos[j] < first_pos[t])
                first_pos[j]++;
        }
        first_pos[t] = 1;
    }
