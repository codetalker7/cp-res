...
/*
	 ...
	 file name: CF1534B
	 date created: 2021-06-23 23:13:52
	 problem link: https://codeforces.com/contest/1534/problem/B
*/
#include<iostream>
#include<vector>
#include<string>
...
void solve(ll mcase){
    ...
    //computing the initial boundary
    ll boundary = a[1];
    for (ll i = 2; i <= n; i++){
        boundary += abs(a[i] - a[i - 1]);
    }
    boundary += a[n];

    //computing the min score
    ll score = boundary;
    for (ll i = 1; i <= n; i++){
        if (i == 1){
            if (a[1] > a[2]){
