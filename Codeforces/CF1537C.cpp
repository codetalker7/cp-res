...
/*
	 ...
	 file name: CF1537C
	 date created: 2021-06-18 21:14:32
...
void solve(ll mcase){
    ...
    //finding the min of the consecutive differences
    ll min_diff = INF;
    for (ll i = 0; i < n - 1; i++){
        if (min_diff > h[i + 1] - h[i])
            min_diff = h[i + 1] - h[i];
    }

    //iterating over all the possible min_diffs and finding the max_ans
    ll max_ans = -INF;
    for (ll i = 0; i < n - 1; i++){
