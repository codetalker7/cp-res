...
/*
	 ...
	 file name: CF1534C
	 date created: 2021-06-23 23:26:03
...
ll partner_vertex[400000 + 1][2];
ll color[400000 + 1][2];
ll row1[400000 + 1];
ll row2[400000 + 1];

void bfs(ll n, ll source){
    /*
        the edges in the graph are: 
        [i][0] -- [i][1]
        [i][0] -- [partner_vertex[i][0]][1]
        [i][1] -- [partner_vertex[i][1]][0]
    */

    queue <pair<ll, ll> > q;
    //source is always [source][0]
    q.push({source, 0});
    color[source][0] = 1;
    ...
    while(!q.empty()){
        ...
        if (color[partner_vertex[s][t]][1 - t] == 0){
            color[partner_vertex[s][t]][1 - t] = 1;
            q.push({partner_vertex[s][t], 1 - t});
        }
    }
}
...
void solve(ll mcase){
    ...
    //taking row1 as input
    for (ll i = 1; i <= n; i++){
        cin >> row1[i];
    }

    //taking row2 as input
    for (ll i = 1; i <= n; i++){
        cin >> row2[i];
    }

    //setting partner_vertex
    for (ll i = 1; i <= n; i++){
        ll u = row1[i], v = row2[i];
        partner_vertex[u][0] = v;
        partner_vertex[v][1] = u; 
    }
