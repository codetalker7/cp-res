...
/*
	 ...
	 file name: CF1509B
	 date created: 2021-06-28 19:46:09
...
void solve(ll mcase){
    ...
    for (ll i = 0; i < n; i++){
        if (s[i] == 'T'){
            ...
            posM.push_back(i);
            assigned_M_left.push_back(0);
            assigned_M_right.push_back(0);
            M++;
        }
    }
    ...
    for (ll i = ssz(posT) - 1, j = 0; j < k; i--, j++){
        if (posT[i] < posM[k - 1 - j]){
            flag = 0;
            break;
        }
    }
