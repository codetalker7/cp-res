...
/*
	 ...
	 file name: CF1529D
	 date created: 2021-05-28 13:21:28
	 problem link: https://codeforces.com/contest/1529/problem/D
*/
#include<iostream>
#include<vector>
#include<string>
...
vll dp(1000001 , 0);
vll num_of_divisors(1000001 , 0);

void find_divisors(){
    for (ll i = 1; i <= 1000000; i++){
        for (ll j = i; j <= 1000000; j += i)
            num_of_divisors[j] = (num_of_divisors[j] + 1) % MOD;
    }
}
...
void solve(ll mcase){
    ...
    ll pref = 2;
    for (ll i = 2; i <= n; i++){
        dp[i] = (pref + num_of_divisors[i] - 1) % MOD;
        pref = (pref + dp[i]) % MOD;
    }
    cout << dp[n];
}
