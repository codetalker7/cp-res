...
/*
	 ...
	 file name: CF1535D
	 date created: 2021-06-04 21:13:40
	 problem link: https://codeforces.com/contest/1535/problem/D
*/
#include<iostream>
#include<vector>
#include<string>
...
void solve(ll mcase){
    ...
    for (ll i = 0; i < k; i++){
        reverse(s.begin() + (1 << i) - 1, s.begin() + (1 << (i + 1)) - 1);
    }
    vll tree((1 << k) , 0);
    ...
    for (ll i = 1; i <= q; i++){
        ...
        cin >> p >> c;
        //computing the position from p
        p = compute_pos(p , k);

        //update things at position p
        s[p - 1] = c;
        for (ll i = p; i >= 1; i >>= 1){
            if (2*i > total){
                if (s[i - 1] == '0' || s[i - 1] == '1')
