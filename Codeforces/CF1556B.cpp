...
/*
	 ...
	 file name: 2.cpp
	 date created: 2021-08-29 20:32:37
...
void caseEven(ll n, ll zeros, ll ones){
    //calculate the gap of a from d1
    ll ans1 = 0, curr = 0;
    for (ll i = 1; i <= n; i++){
        if (a[i] == 0){
            curr++;
...
void caseOdd(ll n, ll zeros, ll ones){
    if (zeros > ones){
        ll ans1 = 0, curr = 0;
        for (ll i = 1; i <= n; i++){
            if (a[i] == 0){
