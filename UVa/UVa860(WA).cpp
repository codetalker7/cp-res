...
/*
	 ...
	 file name: UVa860.cpp
	 date created: 2021-08-21 12:50:26
	 problem link: 
*/
#include<iostream>
#include<vector>
...
*/
    ...
    */
    ...
    while (scanf("%[^,.:;!?\"() \t\n]", input) == 1){
        string cmp = input;
        if (cmp.compare("****END_OF_INPUT****") == 0)
            break;
        else if (cmp.compare("****END_OF_TEXT****") == 0){
            //computing lambda
            double log_lam = log10(lam);
            map <string, ll>::iterator it;

            double entropy = 0;
            for (it = mp.begin(); it != mp.end(); it++){
                entropy += ((double)it->second)*(log_lam - log10(it->second));
            }
            entropy = entropy/((double)lam);
            double relEntropy = (entropy/log_lam) * 100;

            printf("%lld %.1f %lld\n", lam, entropy, (ll)relEntropy);
        ...
        for (ll i = 0; i < ssz(cmp); i++){
            if (cmp[i] <= 'Z' && cmp[i] >= 'A')
                cmp[i] = 'a' + cmp[i] - 'A';
        }

        //if cmp is found before, increment its freq
