...
/*
	 ...
	 file name: UVa417.cpp
	 date created: 2021-08-21 01:25:08
	 problem link: 
*/
#include<iostream>
#include<vector>
...
void preprocess(ll x, ll startIndex, string s){
    if (x == 1){
        for (ll i = startIndex; i < 26; i++){
            string t = s;
            t.push_back('a' + i);
            curr_index++;
            mp[t] = curr_index;
        ...
        for (ll i = startIndex; i < 26; i++){
            string t = s;
            t.push_back('a' + i);
            preprocess(x - 1, i + 1, t);
        }
    }
}


void solve(char input[5]){
    string mstring = input;
    for (ll i = 0; i < ssz(mstring) - 1; i++){
