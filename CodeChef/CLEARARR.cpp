...
/*
	 ...
	 file name: 4.cpp
	 date created: 2021-08-22 21:25:58
...
void solve(ll mcase){
    ...
    ll ans = INF;
    for (ll l = 0; l <= k; l++){
        //removing l pairs with cost x

        //if l pairs can be formed, take the sum of the 2l largest elements
        if (2*l <= n){
            ans = min(ans, prefix[n] + l*x - (prefix[n] - prefix[n - 2*l]));
        }
    }
    printf("%lld\n", ans);
}
