...
/*
	 ...
	 file name: 3.cpp
	 date created: 2021-08-28 19:42:48
...
void solve(ll mcase){
    ...
    //checking if any values greater than a[1] exists 
    for (ll i = 2; i <= n; i++){
        if (a[i] > a[1]){
            printf("-1\n");
            return;
    ...
    ll curr_index = 1, ans = 0;
    while (curr_index < n){
        ans++;
        curr_index = suffix_max[curr_index + 1];
    }
    printf("%lld\n", ans);
}
