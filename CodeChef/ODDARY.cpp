...
/*
	 ...
	 file name: 5.cpp
	 date created: 2021-08-22 21:26:01
	 problem link: https://www.codechef.com/COOK132B/problems/ODDARY
*/
#include<iostream>
#include<vector>
#include<string>
...
void preprocess(){
    mstring.push_back(1);
    ll base_length = 1, curr_max = 1;

    while(base_length < 1000){
        //compute the new base_length

        //increment curr_max
        curr_max++;
        mstring.push_back(curr_max);
        for (ll i = 0; i < base_length; i++){
            mstring.push_back(mstring[i]);
        }

        base_length = 2*base_length + 1;
    }
