...
pair<ll , ll> calculate_k(ll x){
    //c2 is the number of digits in x
    ll temp = 0 , c1 = 1 , c2 = 0;
    while (c1 <= x){
        c2++; c1 <<= 1;
        if (1 << (c2 - 1) & x)
...
void solve(){
    ...
    //we need to maximise c(c + k.first), which is a quadratic function
    //it is not hard to see that k.first will be positive here
    //over all c <= 2^{d - 2}, where d is the number of digits in C
    //calculate k: k.first is the constant, k.second is the number of digits

    auto k = calculate_k(C);
    ll num = 0;
    for (ll i = 0; i <= k.second - 2; i++)
        num += (1 << i);
    cout << num * (num + k.first) << "\n";
}
