...
/*
	 ...
	 file name: THOUSES
	 date created: 2021-05-09 13:10:47
	 problem link: https://www.codechef.com/MAY21C/problems/THOUSES
*/
#include<iostream>
#include<vector>
#include<string>
...
ll get_min_sum(ll node , ll size , ll X){
    if (size == 0){
        return X;
    }
    vll list;
    for (auto u : adj[node]){
        if (color[u] == 0){
            color[u] = 1;
            list.push_back(get_min_sum(u , adj[u].size() - 1 , 1));
        }
    }
    //here, size of list will be size
    //sorting the list
    ...
    for (ll i = 0; i < list.size(); i++)
        total = (total + X * (list.size() - i) * list[i]);
    total += X;
    return total;
}
...
void solve(ll mcase){
    ...
    color[1] = 1;
    cout << ((X % MOD) * (get_min_sum(1 , adj[1].size() , 1) % MOD) % MOD) << "\n";
}
