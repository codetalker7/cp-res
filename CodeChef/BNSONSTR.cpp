...
/*
	 ...
	 file name: BNSONSTR
	 date created: 2021-06-20 21:50:30
...
ll get_ans(ll n, string s, ll d){
    ...
    ll count = INF; 
    for (ll i = 0; i < d; i++){
        count = min(count, n/d - mod_d[i] + total_1 - mod_d[i]);
    }
    return count;
}
...
void solve(ll mcase){
    ...
    //otherwise rebase the string
    string T = "";
    for (ll j = 0; j < N; j++){
        T.push_back(S[(i + j + N) % N]);
    }

    ll ans = INF;
    //checking ans for each divisor d of N
    for (ll d = 1; d <= sqrt(N); d++){
        if (N % d == 0){
            ans = min(ans, get_ans(N, T, d));
            ans = min(ans, get_ans(N, T, N/d));        
        }
    }
    cout << ans << "\n";
}
