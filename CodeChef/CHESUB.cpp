...
/*
	 ...
	 file name: CHESUB
	 date created: 2021-05-31 21:03:01
	 problem link: 
*/
#include<iostream>
#include<vector>
...
void solve(ll mcase){
    ...
    for (ll i = N; i >= 1; i--){
        for (ll j = K; j >= 1; j--){ 
            ...
            if (j == K){
                dp[i][K][1] = max(dp[i + 1][K][1] , K*A[i] + dp[i + 1][K][0]);
                dp[i][K][0] = max((ll)0 , K*A[i] + dp[i + 1][K][0]);
            }
            else{
                dp[i][j][1] = max(dp[i + 1][j][1] , j * A[i] + dp[i + 1][j][0]);
                dp[i][j][1] = max(dp[i][j][1] , j * A[i] + dp[i + 1][j + 1][1]);

                dp[i][j][0] = max(dp[i + 1][j + 1][1] , j*A[i] + dp[i + 1][j][0]);
