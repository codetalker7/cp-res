...
/*
	 ...
	 file name: NUMCOMP1
	 date created: 2021-05-31 20:41:33
	 problem link: https://www.codechef.com/LTIME96B/problems/NUMCOMP1
*/
#include<iostream>
#include<vector>
#include<string>
...
vll primes(10000000 + 1 , 1);
vll primes_till_n(10000000 + 1 , 0);

void precompute(){
    for (ll i = 2; i <= 10000000; i++){
        if (primes[i]){
            primes_till_n[i] = primes_till_n[i - 1] + 1;
            for (ll j = 2 * i; j <= 10000000; j += i)
                primes[j] = 0;
        }
        else{
            primes_till_n[i] = primes_till_n[i - 1];
        }
    }
}
...
void solve(ll mcase){
    ...
    if (N/2 > 1)
        cout << primes_till_n[N] - primes_till_n[N/2] + 1 << "\n";
    else 
        cout << primes_till_n[N] << "\n";
}   
