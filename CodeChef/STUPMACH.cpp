...
/*
	 ...
	 file name: STUPMACH
	 date created: 2021-05-01 13:17:26
*/
#include<iostream>
#include<vector>
#include<string>
...
void solve(ll mcase){
	ll N; cin >> N;
	vll capacity(N + 1) , min_range(N + 1);

	for (ll i = 1; i <= N; i++)
		cin >> capacity[i]; 

	//calculating the minimums in the range capacity[1..j] for every j
	ll temp = INF;
	for (ll i = 1; i <= N; i++){
	...
	//ntimes is the number of operations done overall
	ll ntimes = 0 , count = 0;
	for (ll i = N; i >= 1; i--){
		while(min_range[i] - ntimes > 0){
			ntimes++;
			count += i;
		}
	}
