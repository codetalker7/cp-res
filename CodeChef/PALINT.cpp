...
/*
	 ...
	 file name: PALINT.cpp
	 date created: 2021-09-04 21:54:46
...
void solve(ll mcase){
    ...
    for (ll i = 1; i <= n; i++){
        ...
        max_equal = max(max_equal, freq[a]);
    }

    //max_equal now contains the max frequency
    //now iterate over all possible values in the array

    for (it = freq.begin(); it != freq.end(); it++){
        ll val = it -> first, frequency = it -> second;

        //try to fix val
        it1 = freq.find(val ^ x);
        if (x != 0 AND it1 != freq.end()){
            if (max_equal < frequency + it1 -> second){
                max_equal = frequency + it1 -> second;
                moves = min(it1 -> second, frequency);
            }
            else if (max_equal == frequency + it1 -> second){
                moves = min(moves, it1 -> second);
                moves = min(moves, frequency);
            }
        }
    }
    printf("%lld %lld\n", max_equal, moves);
