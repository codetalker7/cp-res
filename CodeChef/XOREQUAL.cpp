...
/*
	 ...
	 file name: XOREQUAL
	 date created: 2021-05-07 17:07:17
...
void solve(ll mcase){
    ...
    /*it can be checked that the only x that satisfy the given equality
    are those x which have either 000, 010, 100 or 110 as their
    last three binary digits*/
    ...
    /*
        we assume that N >= 3. So, the max number of digits
        we are allowed is N. So, we need to fill in N - 3 digits. The answer 
        then is 4 * 2^{N - 3}
    */
    ll ans = expo<ll>(2 , N - 3 , MOD);
    ans = (4 * ans) % MOD;
    cout << ans << "\n";
}
