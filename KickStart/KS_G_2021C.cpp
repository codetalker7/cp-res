...
/*
	 ...
	 file name: 3.cpp
	 date created: 2021-10-16 19:23:22
	 problem link: https://codingcompetitions.withgoogle.com/kickstart/round/00000000004362d6/00000000008b44ef
*/
#include<iostream>
#include<vector>
#include<string>
...
void solve(ll mcase){
    ...
    forllrev(i, N, 1, 1){
        ...
        forllrev(j, i, 1, 1){
            ...
            curr_sum += B[j];
            if (curr_sum <= K){
                ans = min(ans, i - j + 1 + best[K - curr_sum]);
            }
        }
        ...
        forll(j, i, N, 1){
            curr_sum += B[j];
            if (curr_sum <= K)
                best[curr_sum] = min(best[curr_sum], j - i + 1);
        }
    }
